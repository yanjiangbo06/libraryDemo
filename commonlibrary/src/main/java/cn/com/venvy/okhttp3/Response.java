...
import static cn.com.venvy.okhttp3.internal.http.StatusLine.HTTP_PERM_REDIRECT;
import static cn.com.venvy.okhttp3.internal.http.StatusLine.HTTP_TEMP_REDIRECT;
...
import cn.com.venvy.okhttp3.internal.http.OkHeaders;
import cn.com.venvy.okio.Buffer;
import cn.com.venvy.okio.BufferedSource;
/**
 * An HTTP response. Instances of this class are not immutable: the response
 * body is a one-shot value that may be consumed only once and then closed. All
 * other properties are immutable.
 * 
 * <p>
 * This class implements {@link Closeable}. Closing it simply closes its
 * response body. See {@link ResponseBody} for an explanation and examples.
 */
public final class Response implements Closeable {
	...
	public long getDate() {
		return header("Date", 0);
	}

	public long header(String name, long defaultValue) {
		String result = headers.get(name);
		if (result == null) {
			return defaultValue;
		}
		try {
			return Date.parse(result); // TODO: use HttpDate.parse()
		} catch (Exception e) {
			return defaultValue;
		}
	}
	...
	public ResponseBody peekBody(long byteCount) throws IOException {
		...
		// There may be more than byteCount bytes in source.buffer(). If there
		// is, return a prefix.
		Buffer result;
	 ...
	 * response didn't use the network, such as when the response is fully
	 * cached. The body of the returned response should not be read.
	 */
	...
	/**
	 * Returns the response for the HTTP redirect or authorization challenge
	 * that triggered this response, or null if this response wasn't triggered
	 * by an automatic retry. The body of the returned response should not be
	 * read because it has already been consumed by the redirecting client.
	 */
	...
	/**
	 * Returns the authorization challenges appropriate for this response's
	 * code. If the response code is 401 unauthorized, this returns the
	...
	/**
	 * Returns a {@linkplain System#currentTimeMillis() timestamp} taken
	 * immediately before OkHttp transmitted the initiating request over the
	 * network. If this response is being served from the cache then this is the
	...
	/**
	 * Returns a {@linkplain System#currentTimeMillis() timestamp} taken
	 * immediately after OkHttp received this response's headers from the
	 * network. If this response is being served from the cache then this is the
	...
	public String toString() {
		return "Response{protocol=" + protocol + ", code=" + code
				+ ", message=" + message + ", url=" + request.url() + '}';
	}
	...
	public static class Builder {
		 ...
		 * for multiply-valued headers like "Set-Cookie".
		 */
