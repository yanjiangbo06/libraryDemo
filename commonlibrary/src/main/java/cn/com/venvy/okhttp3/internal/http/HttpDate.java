...
import static cn.com.venvy.okhttp3.internal.Util.UTC;
...
public final class HttpDate {
	...
	private static final ThreadLocal<DateFormat> STANDARD_DATE_FORMAT = new ThreadLocal<DateFormat>() {
		...
		protected DateFormat initialValue() {
			// RFC 2616 specified: RFC 822, updated by RFC 1123 format with
			// fixed GMT.
	...
	public static Date parse(String value) {
		...
		if (position.getIndex() == value.length()) {
			// STANDARD_DATE_FORMAT must match exactly; all text must be
			// consumed, e.g. no ignored
		...
		synchronized (BROWSER_COMPATIBLE_DATE_FORMAT_STRINGS) {
			for (int i = 0, count = BROWSER_COMPATIBLE_DATE_FORMAT_STRINGS.length; i < count; i++) {
				...
				if (format == null) {
					...
					// Set the timezone to use when interpreting formats that
					// don't have a timezone. GMT is
				...
				if (position.getIndex() != 0) {
					// Something was parsed. It's possible the entire string was
					// not consumed but we ignore
					// that. If any of the
					// BROWSER_COMPATIBLE_DATE_FORMAT_STRINGS ended in "'GMT'"
					// we'd have
					// to also check that position.getIndex() == value.length()
					// otherwise parsing might have
					// terminated early, ignoring things like "+01:00". Leaving
					// this as != 0 means that any
