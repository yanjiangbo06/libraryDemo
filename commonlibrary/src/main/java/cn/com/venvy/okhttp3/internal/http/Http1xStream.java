...
import static cn.com.venvy.okhttp3.internal.Util.checkOffsetAndCount;
import static cn.com.venvy.okhttp3.internal.http.StatusLine.HTTP_CONTINUE;
 ...
 * <li>Open a sink to write the request body. Either
 * {@linkplain #newFixedLengthSink fixed-length} or {@link #newChunkedSink
 * chunked}.
 * <li>Write to and then close that sink.
 * <li>{@linkplain #readResponse Read response headers}.
 * <li>Open a source to read the response body. Either
 * {@linkplain #newFixedLengthSource fixed-length},
 * {@linkplain #newChunkedSource chunked} or {@linkplain #newUnknownLengthSource
 * unknown length}.
 * <li>Read from and close that source.
 * </ol>
 *
 * <p>
 * Exchanges that do not have a request body may skip creating and closing the
 * request body. Exchanges that do not have a response body can call
 * {@link #newFixedLengthSource(long) newFixedLengthSource(0)} and may skip
 * reading and closing that source.
 */
public final class Http1xStream implements HttpStream {
	private static final int STATE_IDLE = 0; // Idle connections are ready to
	...
	/**
	 * The stream allocation that owns this stream. May be null for HTTPS proxy
	 * tunnels.
	 */
	...
	public void writeRequestHeaders(Request request) throws IOException {
		...
		String requestLine = RequestLine.get(request, httpEngine.getConnection().route().proxy().type());
	...
	public Response.Builder readResponse() throws IOException {
		...
		try {
			...
			// Provide more context if the server ends the stream before sending
			// a response.
	...
	/**
	 * Sets the delegate of {@code timeout} to {@link Timeout#NONE} and resets
	 * its underlying timeout to the default configuration. Use this to avoid
	 * unexpected sharing of timeouts between pooled connections.
	 */
	...
	private final class FixedLengthSink implements Sink {
		...
		public void flush() throws IOException {
			if (closed)
				return; // Don't throw; this stream might have been closed on
						// the caller's behalf.
			sink.flush();
		}
	...
	private class FixedLengthSource extends AbstractSource {
		...
		public long read(Buffer sink, long byteCount) throws IOException {
			...
			if (read == -1) {
				endOfInput(false); // The server didn't supply the promised
									// content length.
				throw new ProtocolException("unexpected end of stream");
			}
