 ...
 */
package cn.com.venvy.okhttp3.internal.framed;

import static cn.com.venvy.okhttp3.internal.Util.format;
import static cn.com.venvy.okhttp3.internal.framed.Http2.FrameLogger.formatHeader;
import static cn.com.venvy.okio.ByteString.EMPTY;
...
public final class Http2 implements Variant {
	...
	/**
	 * The initial max frame size, applied independently writing to, or reading
	 * from the peer.
	 */
	...
	static final class Reader implements FrameReader {
		...
		public void readConnectionPreface() throws IOException {
			if (client)
				return; // Nothing to read; servers doesn't send a connection
						// preface!
		...
		public boolean nextFrame(Handler handler) throws IOException {
			...
			int streamId = (source.readInt() & 0x7fffffff); // Ignore reserved
															// bit.
			if (logger.isLoggable(FINE))
			...
			default:
				// Implementations MUST discard frames that have unknown or
				// unsupported types.
				source.skip(length);
			}
			return true;
	...
	static final class ContinuationSource implements Source {
		...
		private void readContinuationHeader() throws IOException {
			...
			if (type != TYPE_CONTINUATION)
				throw ioException("%s != TYPE_CONTINUATION", type);
	...
	static final class FrameLogger {
		...
		static String formatHeader(boolean inbound, int streamId, int length, byte type, byte flags) {
			...
			return format("%s 0x%08x %5d %-13s %s", inbound ? "<<" : ">>", streamId, length, formattedType,
					formattedFlags);
		}
		...
		private static final String[] TYPES = new String[] { "DATA", "HEADERS", "PRIORITY", "RST_STREAM", "SETTINGS",
		...
		static {
			...
			for (int frameFlag : frameFlags) {
				for (int prefixFlag : prefixFlags) {
					...
					FLAGS[prefixFlag | frameFlag | FLAG_PADDED] = FLAGS[prefixFlag] + '|' + FLAGS[frameFlag]
							+ "|PADDED";
				}
			}
